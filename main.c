...
int init_all_components(void) {
    ...
    // 启动摄像头守护进程
    int camera_result = system("./camera_service.sh start > /dev/null 2>&1");
    if (camera_result != 0) {
...
void cleanup_all_components(void) {
    ...
    log_info("停止摄像头守护进程...");
    int stop_result = system("./camera_service.sh stop > /dev/null 2>&1");
    if (stop_result == 0) {
        log_info("摄像头守护进程停止成功");
    } else {
        log_warn("摄像头守护进程停止失败 (返回码: %d)", stop_result);
    }
